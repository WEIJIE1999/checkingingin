  ...
  >
    ...
    >
      ...
      <el-form-item label-width="110px" label="参与考勤人员:" prop="allName">
        <el-button>{{ manNumber }}</el-button>
      </el-form-item>
      <el-form-item label-width="85px" label="考勤类型:" prop="radio">
        <div>
          <el-radio v-model="addGroup.radio" label="1"
            >固定班制(每天考勤时间一样，适用于固定上班时间制的员工)</el-radio
          >
        </div>
        <div>
          <el-radio v-model="addGroup.radio" label="2"
      ...
      <el-form-item label="休息日选择:" prop="select">
        <el-checkbox-group v-model="addGroup.select">
          ...
          <el-checkbox
            :checked="this.addGroup.radio === '1' ? true : false"
            label="星期六"
          ></el-checkbox>
          <el-checkbox label="星期日"></el-checkbox>
        </el-checkbox-group>
      </el-form-item>
      <el-calendar v-show="this.addGroup.radio === '1'" v-model="yearData">
      ...
      <el-calendar v-show="this.addGroup.radio === '2'" v-model="yearData">
        <template slot="dateCell" slot-scope="{ date, data }">
          <div
            style="background-color: #F0F0F0;width:100%;height:100%"
            v-if="date.getDay() === 0"
